...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        _, dw1, db1 = affine_backward(drelu, fc1_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.normalization=='batchnorm':
                fc_act, bn_cache[str(i+1)] = batchnorm_forward(fc_act, self.params['gamma'+str(i+1)], 
                                                            self.params['beta'+str(i+1)], self.bn_params[i])
            if self.normalization=='layernorm':
                fc_act, bn_cache[str(i+1)] = layernorm_forward(fc_act, self.params['gamma'+str(i+1)], 
                                                            self.params['beta'+str(i+1)], self.bn_params[i])
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            if self.normalization:
                if self.normalization=='batchnorm':
                    drelu, dgamma, dbeta = batchnorm_backward(drelu, bn_cache[str(i)])
                if self.normalization=='layernorm':
                    drelu, dgamma, dbeta = layernorm_backward(drelu, bn_cache[str(i)])
