...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    running_var = momentum * running_var + (1 - momentum) * var_t
    ...
    if mode == 'train':
        ...
        x_shift = x - mean_t
        normalized_data = x_shift/np.sqrt(var_t + eps)
        out = gamma * normalized_data + beta

        cache = (gamma, x_shift, var_t, normalized_data, eps)
        running_mean = momentum * running_mean + (1 - momentum) * mean_t
        running_var = momentum * running_var + (1-momentum) * var_t
...
def batchnorm_backward(dout, cache):
    ...
    (gamma, x_shift, var_t, x_hat, eps) = cache 
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(x_hat*dout, axis=0)
    N = x_shift.shape[0]

    dx_1 = gamma * dout
    dx_2_a = ((var_t + eps) ** -0.5) * dx_1

    dx_2_b = np.sum(x_shift * dx_1, axis=0)
    dx_3_b = (-0.5) * ((var_t + eps) ** -1.5) * dx_2_b
    dx_4_b = dx_3_b * 1
    dx_5_b = np.ones_like(x_shift) / N * dx_4_b
    dx_6_b = 2 * x_shift * dx_5_b

    dx_7_a = dx_6_b * 1 + dx_2_a * 1
    dx_7_b = dx_6_b * 1 + dx_2_a * 1
    dx_8_b = -1 * np.sum(dx_7_b, axis=0)
    dx_9_b = np.ones_like(x_shift) / N * dx_8_b
    dx_10 = dx_9_b + dx_7_a
    dx = dx_10
...
def batchnorm_backward_alt(dout, cache):
    ...
    N = x_shift.shape[0]
    dx_hat = dout * gamma
    dvar = np.sum(dx_hat * x_shift * -0.5 * np.power(var_t + eps, -1.5), axis = 0)
    dmean = np.sum(dx_hat * -1 / np.sqrt(var_t +eps), axis = 0) + dvar * np.mean(-2 * x_shift, axis =0)
    dx = 1 / np.sqrt(var_t + eps) * dx_hat + dvar * 2.0 / N * x_shift + 1.0 / N * dmean
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    mean_t = np.mean(x, axis=1, keepdims=True)
    var_t = np.var(x, axis=1, keepdims=True)
...
def layernorm_backward(dout, cache):
    ...
    D = x_shift.shape[1]
    dx_hat = dout * gamma
    dvar = np.sum(dx_hat * x_shift * -0.5 * np.power(var_t + eps, -1.5), axis = 1, keepdims=True)
    dmean = np.sum(dx_hat * -1 / np.sqrt(var_t +eps), axis = 1, keepdims=True) + dvar * np.mean(-2 * x_shift, axis =1, keepdims=True)
    dx = 1 / np.sqrt(var_t + eps) * dx_hat + dvar * 2.0 / D * x_shift + 1.0 / D * dmean
      ...
      for fi in range(F):
        for hi in range(H_out):
          for wi in range(W_out):
            cov = w[fi,...]*x_pad[ni,:,hi*stride:hi*stride+HH,wi*stride:wi*stride+WW] 
            out[ni,fi,hi,wi] = np.sum(cov) + b[fi]
      ...
      for fi in range(F):
        for hi in range(H_out):
          for wi in range(W_out):
            ...
            dx_temp[ni,:,hi*stride:hi*stride+HH,wi*stride:wi*stride+WW] += dout[ni, fi, hi, wi] * w[fi, ...]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # ref: https://github.com/haofeixu/stanford-cs231n-2018/blob/master/assignment2/cs231n/layers.py
    N, C, H, W = x.shape
